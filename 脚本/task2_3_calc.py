...
def analyze_homology_representation(sc, name):
    ...
    print(f"==========================================")
    print(f"Simplices: V={m0}, E={m1}, T={m2}, Tet={m3}")
    print(f"Total Betti: beta = ({beta0_tot}, {beta1_tot}, {beta2_tot}, {beta3_tot})")
    print(f"Subspace dims C+: C0+={dim0_p}, C1+={dim1_p}, C2+={dim2_p}, C3+={dim3_p}")
    print(f"Subspace dims C-: C0-={dim0_m}, C1-={dim1_m}, C2-={dim2_m}, C3-={dim3_m}")
    print(f"Betti C+: beta+ = ({beta0_p}, {beta1_p}, {beta2_p}, {beta3_p})")
    print(f"Betti C-: beta- = ({beta0_m}, {beta1_m}, {beta2_m}, {beta3_m})")
    print(f"Sum Betti: beta_tot = ({beta0_p+beta0_m}, {beta1_p+beta1_m}, {beta2_p+beta2_m}, {beta3_p+beta3_m})")
...
def main():
    ...
    # Case 1: Exclude {8}
    sc_ex8 = sc_base.copy()
    existing = sorted(list(sc_ex8.vertices))
    coning_set = [v for v in existing if v != 8]
    sc_ex8.insert_vertex_with_cone(16, coning_set)
    sc_ex8.insert_vertex_with_cone(26, coning_set)
    analyze_homology_representation(sc_ex8, "EXCLUDE PAIR {8}")

    # Case 2: K-Closure
    sc_k = sc_base.copy()
    sc_k.insert_vertex_with_cone(16, existing)
    sc_k.insert_vertex_with_cone(26, existing)
    analyze_homology_representation(sc_k, "K-CLOSURE")
